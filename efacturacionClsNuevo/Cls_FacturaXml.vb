...
Public Class Cls_FacturaXml
    ...
    Public Function CreateInvoice(filename As String, Ruc As String, ByVal Oversion As String, OCustomId As String, OComprobante As String, OFEmsion As DateTime,
        ...
        'Invoice.ProfileID.schemeName = "SUNAT:Identificador de Tipo de Operación"
        ...
        Letra.Value = Cabecera.Rows(0).Item("ImporteLetra") & " " & IIf(MonedaDet = "MN", "SOLES", "DOLARES AMERICANOS") '"CUATROCIENTOS VEINTITRES MIL DOSCIENTOS VEINTICINCO Y 00/100"
        LetraArr(0) = Letra

        Dim placa_vehiculo As String = ""
        If IsDBNull(Cabecera.Rows(0).Item("IdTransportista")) = False Then
            placa_vehiculo = Cabecera.Rows(0).Item("IdTransportista").ToString.Trim   'trans
            If placa_vehiculo.Length > 0 Then
                placa.Value = placa_vehiculo
                LetraArr(1) = placa
            End If
        ...
        With Cabecera.Rows(0)
            ...
            If Not (.Item("TipoDocSunat").ToString.Trim = "1" Or .Item("TipoDocSunat").ToString.Trim = "6") Then
                Invoice.AccountingCustomerParty = Datos_Cli_SEE(.Item("Ubigeo"), .Item("nombreCliente"), "0", "-", .Item("Direccion"), "", .Item("Departamento"), .Item("Provincia"), .Item("Distrito"), "PE", .Item("NombreComercial"))
                'Invoice.AccountingCustomerParty = Datos_Cli_SEE(.Item("Ubigeo"), .Item("nombreCliente"), .Item("TipoDocSunat").ToString.Trim, "-", .Item("Direccion"), "", .Item("Departamento"), .Item("Provincia"), .Item("Distrito"), "PE", .Item("NombreComercial"))
                'Invoice.AccountingCustomerParty = Datos_Cli_SEE(.Item("Ubigeo"), .Item("nombreCliente"), .Item("TipoDocSunat").ToString.Trim, "0", .Item("Direccion"), "", .Item("Departamento"), .Item("Provincia"), .Item("Distrito"), "PE", .Item("NombreComercial"))
            Else
            ...
            If .Item("idformaventa").ToString.Trim = "O" Then
                Tax_pru(0) = Tax_Impuesto(CType(.Item("ValorTotal"), Decimal).ToString("#.00"), CType(0.00, Decimal).ToString("#.00"), .Item("Cod_Tipo_Trib"), .Item("Tipo_Tributo"), .Item("Tax"), .Item("AfecIGV").ToString.Trim, .Item("IGV").ToString.Trim, .Item("tax_Category").ToString.Trim, False, Nothing, CType(.Item("tasa"), Decimal).ToString("#.00"))
            Else
                Tax_pru(0) = Tax_Impuesto(CType(.Item("ValorTotal"), Decimal).ToString("#.00"), CType(.Item("ImporteIGV"), Decimal).ToString("#.00"), .Item("Cod_Tipo_Trib"), .Item("Tipo_Tributo"), .Item("Tax"), .Item("AfecIGV").ToString.Trim, .Item("IGV").ToString.Trim, .Item("tax_Category").ToString.Trim, False, Nothing, CType(.Item("tasa"), Decimal).ToString("#.00"))
            End If
            ...
            '..*************************************
            Dim id_f As New IDType
            Dim payment As New PaymentMeansIDType
            Dim currency As New CurrencyCodeType
            ' Agregar Contado y credito en cuotas segun ver. 
            currency.Value = .Item("idmoneda").ToString.Trim
            id_f.Value = .Item("idformaventa").ToString.Trim
            payment.Value = .Item("Formaventa").ToString.Trim
            Formvta.Value = .Item("Formaventa").ToString.Trim
            FormaVenta.ID = id_f
            FormaVenta.PaymentMeansID = payment
            Formvta1(0) = Formvta
        ...
        Dim invoiceline() As InvoiceLineType
        invoiceline = DetalleItem(detalle.Rows.Count, detalle, placa_vehiculo)
...
#End Region
    ...
    Private Function GetTaxCategory(ByVal Idx As String, Nam_Tax As String, Code_Tax As String, TaxExcepcionCode As String, tipo_impuesto As String, PercentIGV As Decimal, Optional isdetalle As Boolean = True) As TaxCategoryType
        ...
        If isdetalle = True Then
            ...
            'TaxExemptionReasonCode.listName = "SUNAT:Codigo de Tipo de Afectación del IGV"
        ...
        Return TaxCategory
    End Function
    Private Function Tax_Impuesto(ByVal ValorVenta As Decimal, ByVal Tax_importe As Decimal, ByVal IdTax As String, Tax_Name As String, Tax_cod As String, taxExcepCod As String, PercentIGV As Decimal, tax_cate As String, Optional isdetalle As Boolean = True, Optional prodPlas As DataRow = Nothing, Optional tasa_ICBPER As Decimal = 0) As TaxTotalType
        ...
        TaxAmount1.currencyID = ls_TipoMoneda
        ' If Tax_importe <> 0 Then
        Dim TaxSubtotal_plastico As New TaxSubtotalType
        Dim tasub(2) As TaxSubtotalType
        ...
        TaxSubtotal = Tax_SubImpuesto(ValorVenta, Tax_importe, IdTax, Tax_Name, Tax_cod, taxExcepCod, PercentIGV, tax_cate, isdetalle)
        If isdetalle = True Then
            If IsNothing(prodPlas) = False Then
                If prodPlas.Item("IdArticulo").ToString.Trim = "ICBPER" Then
                    TaxSubtotal_plastico = Tax_SubImpuesto_ICBPER(prodPlas.Item("Cantidad"), prodPlas.Item("ICBPER"))
                    TaxTotal1.TaxSubtotal = {TaxSubtotal, TaxSubtotal_plastico}
                Else
                    TaxTotal1.TaxSubtotal = {TaxSubtotal}
                End If
            ...
            End If
        Else
            If tasa_ICBPER <> 0 Then
                TaxSubtotal_plastico = Tax_SubImpuesto_ICBPER(1, tasa_ICBPER, isdetalle)
                TaxTotal1.TaxSubtotal = {TaxSubtotal, TaxSubtotal_plastico}
            Else
                TaxTotal1.TaxSubtotal = {TaxSubtotal}
            End If
    ...
    Private Function Tax_SubImpuesto_ICBPER(ByVal cantidad As Decimal, ByVal tax_tot_plastico As Decimal, Optional isdetalle As Boolean = True) As TaxSubtotalType
        Dim TaxSubtotal As New TaxSubtotalType
        Dim TaxAmount As New TaxAmountType
        Dim BaseUnitMeasure As New BaseUnitMeasureType

        If isdetalle = True Then
            BaseUnitMeasure.Value = CType(cantidad, Decimal).ToString("#.00")
            Dim UnidadMed As Integer
            UnidadMed = DirectCast([Enum].Parse(GetType(UnitCodeContentType), "NIU"), Integer)
            BaseUnitMeasure.unitCode = UnidadMed
            TaxSubtotal.BaseUnitMeasure = BaseUnitMeasure
        End If
        TaxAmount.currencyID = ls_TipoMoneda
        TaxAmount.Value = CType(tax_tot_plastico, Decimal).ToString("#.00")
        TaxSubtotal.TaxAmount = TaxAmount

        TaxSubtotal.TaxCategory = GetTaxCategory_ICBPER(tax_tot_plastico / cantidad, isdetalle)


        Return TaxSubtotal
    End Function
    Private Function GetTaxCategory_ICBPER(impuesto_unitario As Decimal, Optional isdetalle As Boolean = True) As TaxCategoryType
        Dim Name As New NameType1
        Dim TaxTypeCode As New TaxTypeCodeType
        Dim TaxCategory As New TaxCategoryType
        Dim PerUnit As New PerUnitAmountType
        Dim ID_taxscheme As New IDType

        If isdetalle = True Then
            PerUnit.Value = CType(impuesto_unitario, Decimal).ToString("#.00")
            PerUnit.currencyID = ls_TipoMoneda
            TaxCategory.PerUnitAmount = PerUnit
        End If
        Name.Value = "ICBPER"
        ID_taxscheme.Value = "7152"
        TaxTypeCode.Value = "OTH"
        Dim TaxScheme As New TaxSchemeType With {.ID = ID_taxscheme, .Name = Name, .TaxTypeCode = TaxTypeCode}
        TaxScheme.ID.schemeID = "UN/ECE 5153"
        TaxScheme.ID.schemeURI = "urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo05"
        TaxScheme.ID.schemeName = "Codigo de tributos"
        TaxScheme.ID.schemeAgencyName = "PE:SUNAT"
        TaxCategory.TaxScheme = TaxScheme
        Return TaxCategory
    ...
    Private Function ComercialName(ByVal NomComercial As String) As PartyNameType
        ...
        Return PartyName1
    End Function
    Private Function DetalleItem(item As Integer, ByVal datos As DataTable, idplaca As String) As InvoiceLineType()
        Dim InvoiceItem(item) As InvoiceLineType
        Dim i As Integer = 0
        For Each r As DataRow In datos.Rows
            InvoiceItem(i) = DetalleFactura(CInt(r.Item("Item")), r, idplaca)
            i += 1
        Next
        Return InvoiceItem
    End Function
    Private Function DetalleFactura(item As Integer, d As DataRow, idplaca As String) As InvoiceLineType
        ...
        Dim PriceTypeCode As String = IIf(d.Item("IdFormaVenta").ToString.Trim = "O", "02", "01")
        ...
        Tax_pru(1) = Tax_Impuesto(TprecioItem, CType(d.Item("ImporteIGV"), Decimal).ToString("#.00"), d.Item("Cod_Tipo_Trib"), d.Item("Tipo_Tributo"), d.Item("tax"), d.Item("AfecIGV").ToString.Trim, CType(d.Item("IGV"), Decimal).ToString("#.00"), d.Item("tax_category").ToString.Trim, True, d)

        InvoiceLine1.TaxTotal = Tax_pru
        InvoiceLine1.Item = itemarticulo(d.Item("IdArticulo").ToString.Trim, d.Item("Descripcion").ToString.Trim, idplaca, d.Item("Tipo_Tributo"))
    ...
    Private Function itemarticulo(ByVal IdArticulo As String, DescripcionArt As String, idplaca As String, Optional Tipo_Tributo As String = "IGV") As ItemType
        ...
        If IsNothing(art.IdLinea) = False Then
            If art.IdLinea.Trim.Length = 8 Then
                ...
                'Else
                '    MsgBox("El producto no esta configurado para operaciones de exportación, asignar el tipo de linea según sunat" & vbNewLine & " en catalogo de Articulo, ¡¡Se trunco la generación del xml!!", MsgBoxStyle.Critical)
                '    Exit Function
            End If
        End If
        ...
        '*********** PLACA DEL VEHICULO
        Dim aditionaitemproperty_placa As New ItemPropertyType
        Dim name_placa As New NameType1
        Dim name_placa_code As New NameCodeType
        Dim nro_placa As New ValueType

        If idplaca <> "" Then
            name_placa.Value = "Gastos Art. 37 Renta: Número de Placa"
            aditionaitemproperty_placa.Name = name_placa
            name_placa_code.listName = "propiedad del item"
            'name_placa_code.listName = "SUNAT :Identificador de la propiedad del ítem"
            name_placa_code.listAgencyName = "PE:SUNAT"
            name_placa_code.Value = "7000"
            nro_placa.Value = idplaca
            aditionaitemproperty_placa.Name = name_placa
            aditionaitemproperty_placa.NameCode = name_placa_code
            aditionaitemproperty_placa.Value = nro_placa
            Itemx.AdditionalItemProperty = {aditionaitemproperty_placa}
        End If
