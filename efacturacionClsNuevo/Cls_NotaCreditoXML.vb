...
Public Class Cls_NotaCreditoXML
    ...
    Public Function CreateInvoice(filename As String, Ruc As String, ByVal Oversion As String, OCustomId As String, OComprobante As String, OFEmsion As DateTime,
        ...
        With Cabecera.Rows(0)
            ...
            If .Item("idformaventa").ToString.Trim = "O" Then
                Tax_pru(0) = Tax_Impuesto(CType(.Item("ValorTotal"), Decimal).ToString("#.00"), CType(0.00, Decimal).ToString("#.00"), .Item("Cod_Tipo_Trib"), .Item("Tipo_Tributo"), .Item("Tax"), .Item("AfecIGV").ToString.Trim, .Item("IGV").ToString.Trim, .Item("tax_Category").ToString.Trim, False)
            Else
                Tax_pru(0) = Tax_Impuesto(CType(.Item("ValorTotal"), Decimal).ToString("#.00"), CType(.Item("ImporteIGV"), Decimal).ToString("#.00"), .Item("Cod_Tipo_Trib"), .Item("Tipo_Tributo"), .Item("Tax"), .Item("AfecIGV").ToString.Trim, .Item("IGV").ToString.Trim, .Item("tax_Category").ToString.Trim, False)
            End If
...
#End Region
    ...
    Private Function Datos_Prov_SEE(ByVal Ubigeo As String, ByVal razonsocial As String, ByVal TdSunat As String, ByVal IdRuc As String, ByVal Direccion As String, ByVal Urbanizacion As String, ciudad As String, provincia As String, distrito As String, CodPais As String, nomComercial As String, Cod_Estable_Sunat As String) As SupplierPartyType
        ...
        AddressTypeCode1.Value = Cod_Estable_Sunat   '"0001"
    ...
    Private Function DetalleItem(item As Integer, ByVal datos As DataTable) As CreditNoteLineType()
        ...
        '        'End If
        '        InvoiceItem(x) = DetalleFactura(.Rows(x).Item("Item"), .Rows(x))
        '    End With
        'Next
        Dim i As Integer = 0
        ...
        Return InvoiceItem
    End Function
    Private Function DetalleFactura(item As Integer, d As DataRow) As CreditNoteLineType
        Dim InvoiceLine1 As New CreditNoteLineType
        Dim InvoicedQuantity As New CreditedQuantityType
        ...
        ID.Value = item
        InvoiceLine1.ID = ID
        InvoiceLine1.CreditedQuantity = InvoicedQuantity
        ...
        Dim Tax_pru(2) As TaxTotalType
        'If Isc > 0 Then
        '    Tax_pru(0) = Tax_Impuesto(TprecioItem, CType(0.00, Decimal).ToString("#.00"), "2000", "ISC", "EXC", "01", CType(0.00, Decimal).ToString("#.00"))
        'End If
        ' If Igv >= 0 Then
        ...
        'End If
        'If Otros > 0 Then
        '    Tax_pru(2) = Tax_Impuesto(TprecioItem, CType(0.00, Decimal).ToString("#.00"), "9999", "OTROS", "OTH", taxExcepCod, CType(0.00, Decimal).ToString("#.00"))
        'End If
