...
Public Class Cls_FacturaPercepcion
    ...
    Public Sub Pro_Moneda(ByVal IdMon As String)
        ...
        MonedaDet = IdMon
    End Sub
    Public Sub CreatePOFle(filename As String, Ruc As String, ByVal Oversion As String, OCustomId As String, OComprobante As String, OFEmsion As DateTime,
        ...
        With Cabecera.Rows(0)
            ...
            Else
                ...
                If .Item("IsPercepcion") = True Then
                    LeyendaItem(1) = MontoLetras("2000", "COMPROBANTE DE PERCEPCION")
                    MonetaryItem(2) = MontoPercepcion("2001", CType(.Item("percepcion"), Decimal).ToString("#.00"), CType(.Item("ImporteTotal"), Decimal).ToString("#.00"), CType(.Item("ImporteTotal") + .Item("Percepcion"), Decimal).ToString("#.00"))
                End If
    ...
    End Function
    Private Function MontoPercepcion(ByVal ID_ As String, PayableAmount_ As Decimal, ReferenceAmount_ As Decimal, TotalAmount_ As Decimal) As AdditionalMonetaryTotalType
        Dim AdditionalMonetaryTotal1 As New AdditionalMonetaryTotalType
        Dim ID As New IDType
        Dim PayableAmount As New PayableAmountType
        Dim ReferenceAmount As New AmountType1
        Dim TotalAmount As New AmountType1
        ID.Value = ID_
        PayableAmount.currencyID = ls_TipoMoneda
        PayableAmount.Value = PayableAmount_

        ReferenceAmount.currencyID = ls_TipoMoneda
        ReferenceAmount.Value = ReferenceAmount_

        TotalAmount.currencyID = ls_TipoMoneda
        TotalAmount.Value = TotalAmount_

        AdditionalMonetaryTotal1.ID = ID
        AdditionalMonetaryTotal1.PayableAmount = PayableAmount
        AdditionalMonetaryTotal1.ReferenceAmount = ReferenceAmount
        AdditionalMonetaryTotal1.TotalAmount = TotalAmount
        Return AdditionalMonetaryTotal1
    End Function
