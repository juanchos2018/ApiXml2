...
Public Class Cls_Boleta
    ...
    Public Function desce() As String()
        Dim x As New XmlSerializer(GetType(ApplicationResponseType))
        Dim objStreamReader As New StreamReader("D:\Proyecto\SICOVEN-PROTOTIPO\bin\x86\Debug\xmlEnviados\R-20532452261-01-F001-0000004.xml")
        Dim Valores(10) As String
        Dim IDx As New IDType
        ...
        'Dim refId As New ReferenceIDType
        'Dim GetValor(0) As ReferenceIDType
        'refId = ax.Response.ReferenceID
        'GetValor = refId.Value
        'SReponCod = ax.Response.ResponseCode.Value

        'Dim DESX As New DescriptionType
        'DESX = ax.Response.Description.GetValue(0)
        'DESX.Value
        Return Valores

    End Function
    Public Sub CreatePOFile(filename As String, Ruc As String, ByVal Oversion As String, OCustomId As String, OComprobante As String, OFEmsion As DateTime,
        ...
        With Cabecera.Rows(0)
            ...
            If .Item("Ruc").ToString.Trim = "0001" Then
                'Invoice.AccountingCustomerParty = Datos_Cli_SEE("-", "-", "-", "-") 'AccountingCustomerParty
                Invoice.AccountingCustomerParty = Datos_Cli_SEE("-", "-", .Item("nombreCliente"), "-") 'AccountingCustomerParty
            Else
                Invoice.AccountingCustomerParty = Datos_Cli_SEE(.Item("TipoDocSunat"), .Item("Ruc"), .Item("nombreCliente"), .Item("Direccion").ToString.Trim & "-" & .Item("Distrito").ToString.Trim & "-" & .Item("Provincia").ToString.Trim & "-" & .Item("departamento").ToString.Trim) 'AccountingCustomerParty
            End If
    ...
    Private Function Datos_Cli_SEE(ByVal TdSunat As String, ByVal Ruc_Cli As String, razonSocial As String, DireccionCli As String) As Object
        ...
        Dim domicilio As New LocationType1
        Dim direccion As New DescriptionType
        direccion.Value = DireccionCli
        domicilio.Description = direccion
        oParty_Custom.PhysicalLocation = domicilio
    ...
    Private Function DetalleItem(item As Integer, ByVal datos As DataTable) As InvoiceLineType()
        ...
        For x As Integer = 0 To item - 1
            With datos
                ...
                '    InvoiceItem(x) = DetalleFactura(.Rows(x).Item("Item"), .Rows(0).Item("UndRef"), CType(.Rows(x).Item("Cantidad"), Decimal).ToString("#.00"), CType(.Rows(x).Item("ImporteMN"), Decimal).ToString("#.00"), IIf(.Rows(x).Item("IdFormaVenta").ToString.Trim = "O", "02", "01"), CType(.Rows(x).Item("PrecioVenta"), Decimal).ToString("#.00"), CType(0.00, Decimal).ToString("#.00"), CType(0.00, Decimal).ToString("#.00"), CType(0.00, Decimal).ToString("#.00"), .Rows(x).Item("IdArticulo").ToString.Trim, .Rows(x).Item("Descripcion").ToString.Trim, CType(0.00, Decimal).ToString("#.00"), .Rows(x).Item("AfecIGV").ToString.Trim, CType(0.00, Decimal).ToString("#.00"))
                'Else
                '    InvoiceItem(x) = DetalleFactura(.Rows(x).Item("Item"), .Rows(0).Item("UndRef"), CType(.Rows(x).Item("Cantidad"), Decimal).ToString("#.00"), CType(.Rows(x).Item("ImporteMN"), Decimal).ToString("#.00"), IIf(.Rows(x).Item("IdFormaVenta").ToString.Trim = "O", "02", "01"), CType(.Rows(x).Item("PrecioVenta"), Decimal).ToString("#.00"), CType(0.00, Decimal).ToString("#.00"), CType(.Rows(x).Item("ImporteIGV"), Decimal).ToString("#.00"), CType(0.00, Decimal).ToString("#.00"), .Rows(x).Item("IdArticulo").ToString.Trim, .Rows(x).Item("Descripcion").ToString.Trim, CType(.Rows(x).Item("PrecioSIGV"), Decimal).ToString("#.00"), .Rows(x).Item("AfecIGV").ToString.Trim, .Rows(x).Item("IGV").ToString.Trim)
                'End If
                If .Rows(x).Item("IdFormaVenta").ToString.Trim = "O" Then
                    InvoiceItem(x) = DetalleFactura(.Rows(x).Item("Item"), .Rows(x).Item("UndRef"), CType(.Rows(x).Item("Cantidad"), Decimal).ToString("#.00"), CType(IIf(MonedaDet.Trim = "MN", .Rows(x).Item("ImporteMN"), .Rows(x).Item("ImporteUS")), Decimal).ToString("#.00"), IIf(.Rows(x).Item("IdFormaVenta").ToString.Trim = "O", "02", "01"), CType(.Rows(x).Item("PrecioVenta"), Decimal).ToString("#.00"), CType(0.00, Decimal).ToString("#.00"), CType(0.00, Decimal).ToString("#.00"), CType(0.00, Decimal).ToString("#.00"), .Rows(x).Item("IdArticulo").ToString.Trim, .Rows(x).Item("Descripcion").ToString.Trim, CType(0.00, Decimal).ToString("#.00"), .Rows(x).Item("AfecIGV").ToString.Trim, CType(0.00, Decimal).ToString("#.00"))
                Else
                    InvoiceItem(x) = DetalleFactura(.Rows(x).Item("Item"), .Rows(x).Item("UndRef"), CType(.Rows(x).Item("Cantidad"), Decimal).ToString("#.00"), CType(IIf(MonedaDet.Trim = "MN", .Rows(x).Item("ImporteMN"), .Rows(x).Item("ImporteUS")), Decimal).ToString("#.00"), IIf(.Rows(x).Item("IdFormaVenta").ToString.Trim = "O", "02", "01"), CType(.Rows(x).Item("PrecioVenta"), Decimal).ToString("#.00"), CType(0.00, Decimal).ToString("#.00"), CType(.Rows(x).Item("ImporteIGV"), Decimal).ToString("#.00"), CType(0.00, Decimal).ToString("#.00"), .Rows(x).Item("IdArticulo").ToString.Trim, .Rows(x).Item("Descripcion").ToString.Trim, CType(.Rows(x).Item("PrecioSIGV"), Decimal).ToString("#.00"), .Rows(x).Item("AfecIGV").ToString.Trim, .Rows(x).Item("IGV").ToString.Trim)
                End If
            End With
        Next
        Return InvoiceItem
    End Function
    Private Function DetalleFactura(item As Integer, UnidadMedida As String, ByVal cantidad As Decimal, TprecioItem As Decimal, PriceTypeCode As String, PrecioUnit As Decimal, Isc As Decimal, Igv As Decimal, Otros As Decimal, ByVal IdArticulo As String, DescripcionArt As String, precionsinTax As Decimal, taxExcepCod As String, PercentIGV As Decimal) As InvoiceLineType
