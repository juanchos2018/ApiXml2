...
Public Class Cls_ResumenDiarioV11
        ...
        For x As Integer = 0 To item - 1
            With datos
                InvoiceItem(x) = DetalleFactura(x + 1, .Rows(x).Item("TdSunat").ToString.Trim,
                                                ...
                                                .Rows(x).Item("NumeroDocumento").ToString.Trim,
                                                ...
                                                .Rows(x).Item("IdMoneda").ToString.Trim, .Rows(x).Item("TipoDocSunat").ToString.Trim,
                                                .Rows(x).Item("IdCliente").ToString.Trim, .Rows(x).Item("Estado").ToString.Trim, .Rows(x).Item("IdTipoDocSunat").ToString.Trim, .Rows(x).Item("NumeroDocumento2").ToString.Trim, CType(.Rows(x).Item("ICBPER"), Decimal).ToString("#.00"))
            End With
        Next
        Return InvoiceItem
    End Function
    Private Function Datos_Cli_SEE(ByVal TdSunat As String, ByVal Ruc_Cli As String) As Object
    ...
    Private Function DetalleFactura(Linea As String, TipoDoc As String, Serie As String, numero As String, TotalG As Decimal,
                                    Isc As Decimal, Igv As Decimal, Otros As Decimal, OpGrav As Decimal, OpEx As Decimal, OpInf As Decimal,
                                    otrosCargos As Decimal, istrue As Boolean, idmoneda As String, idSunat As String, idcliente As String, estado As String, idtipodocsunatRef As String, NroDocRef As String, Optional impuestoPlastio As Decimal = 0) As SummaryDocumentsLineType
        ...
        Dim IDT As New IDType
        IDT.Value = Serie & "-" & numero '"B001-00000001"
        InvoiceLine1.ID = IDT

        If idcliente.ToString.Trim = "0001" Then
            ''InvoiceLine1.AccountingCustomerParty = Datos_Cli_SEE("-", "-") 'AccountingCustomerParty
            'InvoiceLine1.AccountingCustomerParty = Datos_Cli_SEE("-", "0") 'AccountingCustomerParty
            InvoiceLine1.AccountingCustomerParty = Datos_Cli_SEE(idSunat, "0") 'AccountingCustomerParty
        Else
            InvoiceLine1.AccountingCustomerParty = Datos_Cli_SEE(idSunat, idcliente) 'AccountingCustomerParty
        End If
        If idtipodocsunatRef.Length > 0 Then
            Dim BillingReference As New BillingReferenceType
            Dim InvoiceDocumentReference As New DocumentReferenceType
            Dim IDRef As New IDType
            IDRef.Value = NroDocRef
            Dim TipoDocumentoRef As New DocumentTypeCodeType
            TipoDocumentoRef.Value = idtipodocsunatRef
            InvoiceDocumentReference.ID = IDRef
            InvoiceDocumentReference.DocumentTypeCode = TipoDocumentoRef

            BillingReference.InvoiceDocumentReference = InvoiceDocumentReference
            InvoiceLine1.BillingReference = BillingReference
        End If



        Dim status As New StatusType
        Dim conditioncode As New ConditionCodeType
        conditioncode.Value = estado

        status.ConditionCode = conditioncode

        InvoiceLine1.Status = status
        ...
        'plastico
        'If impuestoPlastio <> 0 Then
        '    bill1(3) = Billin(impuestoPlastio, "03", Moneda)
        'End If
        ...
        '  Tax_pru(3) = Tax_Impuesto(impuestoPlastio, "7152", "ICBPER", "OTH", False, 0, Moneda)
