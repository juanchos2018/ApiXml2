...
Public Class Cls_NotaCredito
    ...
    Public Function CreatePOFile(filename As String, Ruc As String, ByVal Oversion As String, OCustomId As String, OComprobante As String, OFEmsion As DateTime,
        ...
        Dim serializer As New XmlSerializer(GetType(CreditNoteType))
        '**********
        Dim writer As New StreamWriter(filename, False, System.Text.Encoding.GetEncoding("ISO-8859-1"))
        Dim Invoice As New CreditNoteType()
        ...
        With Cabecera.Rows(0)
            ...
            'LineExtensionAmount.Value = .Item("ValorTotal")
            '' impuesto IGV
            'TaxExclusiveAmount.currencyID = ls_TipoMoneda
            'TaxExclusiveAmount.Value = .Item("ImporteIGV")
        ...
        '*************** DETALLE DE FACTURAS
        'detalle 
        Invoice.CreditNoteLine = DetalleItem(detalle.Rows.Count, detalle)
        '*************** Escribe el xml
    ...
    Private Function DetalleItem(item As Integer, ByVal datos As DataTable) As CreditNoteLineType()
        Dim InvoiceItem(item) As CreditNoteLineType
        For x As Integer = 0 To item - 1
            With datos
                ...
                '    InvoiceItem(x) = DetalleFactura(.Rows(x).Item("Item"), .Rows(0).Item("UndRef"), CType(.Rows(x).Item("Cantidad"), Decimal).ToString("#.00"), CType(IIf(monedadet = "MN", .Rows(x).Item("ImporteMN"), .Rows(x).Item("ImporteUS")), Decimal).ToString("#.00"), IIf(.Rows(x).Item("IdFormaVenta").ToString.Trim = "O", "02", "01"), CType(.Rows(x).Item("PrecioVenta"), Decimal).ToString("#.00"), CType(0.00, Decimal).ToString("#.00"), CType(0.00, Decimal).ToString("#.00"), CType(0.00, Decimal).ToString("#.00"), .Rows(x).Item("IdArticulo").ToString.Trim, .Rows(x).Item("Descripcion").ToString.Trim, CType(0.00, Decimal).ToString("#.00"), .Rows(x).Item("AfecIGV").ToString.Trim, CType(0.00, Decimal).ToString("#.00"))
                'Else
                '    InvoiceItem(x) = DetalleFactura(.Rows(x).Item("Item"), .Rows(0).Item("UndRef"), CType(.Rows(x).Item("Cantidad"), Decimal).ToString("#.00"), CType(IIf(monedadet = "MN", .Rows(x).Item("ImporteMN"), .Rows(x).Item("ImporteUS")), Decimal).ToString("#.00"), IIf(.Rows(x).Item("IdFormaVenta").ToString.Trim = "O", "02", "01"), CType(.Rows(x).Item("PrecioVenta"), Decimal).ToString("#.00"), CType(0.00, Decimal).ToString("#.00"), CType(.Rows(x).Item("ImporteIGV"), Decimal).ToString("#.00"), CType(0.00, Decimal).ToString("#.00"), .Rows(x).Item("IdArticulo").ToString.Trim, .Rows(x).Item("Descripcion").ToString.Trim, CType(.Rows(x).Item("PrecioSIGV"), Decimal).ToString("#.00"), .Rows(x).Item("AfecIGV").ToString.Trim, .Rows(x).Item("IGV").ToString.Trim)
                'End If
        ...
        Return InvoiceItem
    End Function
    Private Function DetalleFactura(item As Integer, cantidad As Decimal, TprecioItem As Decimal, PriceTypeCode As String, PrecioUnit As Decimal, Isc As Decimal, Igv As Decimal, Otros As Decimal, ByVal IdArticulo As String, DescripcionArt As String, precionsinTax As Decimal) As CreditNoteLineType
        Dim InvoiceLine1 As New CreditNoteLineType
        Dim InvoicedQuantity1 As New CreditedQuantityType
        ...
        Id.Value = item
        InvoiceLine1.ID = Id
        InvoiceLine1.CreditedQuantity = InvoicedQuantity1
