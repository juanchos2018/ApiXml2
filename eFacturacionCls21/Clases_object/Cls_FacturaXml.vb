...
Public Class Cls_FacturaXml
    ...
    Public Function CreateInvoice(filename As String, Ruc As String, ByVal Oversion As String, OCustomId As String, OComprobante As String, OFEmsion As DateTime,
                         OTd As String, Moneda As String, ByVal DatosEE As DataTable, ByVal Cabecera As DataTable, ByVal detalle As DataTable, TipoOperacion As String) As Byte()
        ...
        pfid.Value = "0101"    'Tabla 51 nuevo tag  implementar en formulario
        ...
        Letra.Value = "CUATROCIENTOS VEINTITRES MIL DOSCIENTOS VEINTICINCO Y 00/100"
        LetraArr(0) = Letra
        ...
        With Cabecera.Rows(0)
            ...
            Dim AllowanceChargeReasonCode As New AllowanceChargeReasonCodeType
            ...
            If .Item("idformaventa").ToString.Trim = "O" Then
                Tax_pru(0) = Tax_Impuesto(CType(.Item("ValorTotal"), Decimal).ToString("#.00"), CType(0.00, Decimal).ToString("#.00"), "1000", "IGV", "VAT", .Item("AfecIGV").ToString.Trim, .Item("IGV").ToString.Trim)
            Else
                Tax_pru(0) = Tax_Impuesto(CType(.Item("ValorTotal"), Decimal).ToString("#.00"), CType(.Item("ImporteIGV"), Decimal).ToString("#.00"), "1000", "IGV", "VAT", .Item("AfecIGV").ToString.Trim, .Item("IGV").ToString.Trim)
            End If
            ...
            Else
                ...
                TaxInclusiveAmount.Value = CType(.Item("ImporteIGV"), Decimal).ToString("#.00")
...
#Region "Atributos"
    ...
    Public Property Name As NameType1
        Get
            Return _Name
        End Get
        Set(value As NameType1)
            _Name = value
        End Set
    End Property
...
#End Region
    ...
    Private Function SignatoryParty(ByVal Ir_Ruc As String, Name_Emision As String) As PartyType
        ...
        Return sigparty
    End Function
    Private Function GetTaxCategory(ByVal Idx As String, Nam_Tax As String, Code_Tax As String, TaxExcepcionCode As String, tipo_impuesto As String, PercentIGV As Decimal) As TaxCategoryType
        Dim ID As New IDType
        ...
        If PercentIGV <> 0.0 Then
            TaxCategory.Percent = New PercentType1 With {.Value = PercentIGV}
        End If
        ...
        TaxScheme.ID.schemeName = "Tax Scheme Identifier"
        TaxScheme.ID.schemeAgencyName = "United Nations Economic Commission for Europe"
        TaxCategory.TaxScheme = TaxScheme
        Return TaxCategory
    End Function
    Private Function Tax_Impuesto(ByVal ValorVenta As Decimal, ByVal Tax_importe As Decimal, ByVal IdTax As String, Tax_Name As String, Tax_cod As String, taxExcepCod As String, PercentIGV As Decimal) As TaxTotalType
        TaxAmount.currencyID = ls_TipoMoneda
        TaxAmount.Value = Tax_importe
        TaxTotal.TaxAmount = TaxAmount
        TaxSubtotal = Tax_SubImpuesto(ValorVenta, Tax_importe, IdTax, Tax_Name, Tax_cod, taxExcepCod, PercentIGV)
        TaxTotal.TaxSubtotal = {TaxSubtotal}
        Return TaxTotal
    End Function
    Private Function Tax_SubImpuesto(ByVal ValorVenta As Decimal, ByVal Tax_SubImporte As Decimal, ByVal IdTax As String, Tax_Name As String, Tax_cod As String, taxExcepCod As String, PercentIGV As Decimal) As TaxSubtotalType
        ...
        TaxSubtotal.TaxCategory = GetTaxCategory(IdTax, Tax_Name, Tax_cod, taxExcepCod, "S", PercentIGV)
    ...
    Private Function Datos_Cli_SEE(ByVal Ubigeo As String, ByVal razonsocial As String, ByVal TdSunat As String, ByVal IdRuc As String, ByVal Direccion As String, ByVal Urbanizacion As String, ciudad As String, provincia As String, distrito As String, CodPais As String, nomComercial As String) As CustomerPartyType
        ...
        CompanyID.schemeName = "SUNAT:Identificador de Documento de Identidad"
    ...
    End Function
    Private Function Datos_Prov_SEE(ByVal Ubigeo As String, ByVal razonsocial As String, ByVal TdSunat As String, ByVal IdRuc As String, ByVal Direccion As String, ByVal Urbanizacion As String, ciudad As String, provincia As String, distrito As String, CodPais As String, nomComercial As String) As SupplierPartyType
        Dim SupplierParty As New SupplierPartyType
        Dim Party As New PartyType
        Dim AdditionalAccountID As New AdditionalAccountIDType
        Dim RegistrationName As New RegistrationNameType
        Dim CompanyID As New CompanyIDType
        ...
        Dim ID As New IDType
        Dim PartyTaxSchemelist() As PartyTaxSchemeType
        Dim PartyTaxScheme As New PartyTaxSchemeType
        Dim PartyIdentification As New PartyIdentificationType
        Dim ID_PartyIdentification As New IDType
        ID_PartyIdentification.Value = IdRuc
        ID_PartyIdentification.schemeID = TdSunat.Trim
        ...
        Party.PartyIdentification = {PartyIdentification}
        Dim PartyLegalEntity As New PartyLegalEntityType
        Party.PartyLegalEntity = {f_PartyLegalEntity_Prov(razonsocial, "0000", Direccion, ciudad, ciudad, distrito, "PE", Ubigeo)}
        '000 AS ESTABLECIMIENTO FISCAL, otros codigos segun establecimientos anexo.
        RegistrationName.Value = razonsocial
        CompanyID.Value = IdRuc.Trim
        CompanyID.schemeID = TdSunat.Trim
        ...
        PartyTaxSchemelist = {PartyTaxScheme}
        Party.PartyTaxScheme = PartyTaxSchemelist
        Party.PartyName = {Prov_ComercialName(nomComercial)}
    ...
    Private Function DetalleFactura(item As Integer, UnidadMedida As String, cantidad As Decimal, TprecioItem As Decimal, PriceTypeCode As String, PrecioUnit As Decimal, Isc As Decimal, Igv As Decimal, Otros As Decimal, ByVal IdArticulo As String, DescripcionArt As String, precionsinTax As Decimal, taxExcepCod As String, PercentIGV As Decimal) As InvoiceLineType
        ...
        InvoicedQuantity.unitCodeListAgencyName = "Europe"
        InvoicedQuantity.Value = cantidad

        LineExtensionAmount.currencyID = ls_TipoMoneda
        LineExtensionAmount.Value = TprecioItem
        ...
        ID.Value = item
        InvoiceLine.ID = ID
        InvoiceLine.InvoicedQuantity = InvoicedQuantity
        InvoiceLine.LineExtensionAmount = LineExtensionAmount
        InvoiceLine.PricingReference = PricingReference

        Dim Tax_pru(2) As TaxTotalType
        If Isc > 0 Then
            Tax_pru(0) = Tax_Impuesto(TprecioItem, Isc, "2000", "ISC", "EXC", "01", CType(0.00, Decimal).ToString("#.00"))
        End If
        If Igv >= 0 Then
            Tax_pru(1) = Tax_Impuesto(TprecioItem, Igv, "1000", "IGV", "VAT", taxExcepCod, PercentIGV)
        End If
        If Otros > 0 Then
            Tax_pru(2) = Tax_Impuesto(TprecioItem, Otros, "9999", "OTROS", "OTH", taxExcepCod, CType(0.00, Decimal).ToString("#.00"))
        End If

        InvoiceLine.TaxTotal = Tax_pru
        InvoiceLine.Item = itemarticulo(IdArticulo, DescripcionArt)
        InvoiceLine.Price = Precio_SinIGv(precionsinTax)
    ...
    Private Function Precio_SinIGv(ByVal PrecionSinTax As Decimal) As PriceType
        ...
        PriceAmount.Value = PrecionSinTax
    ...
    Private Function itemarticulo(ByVal IdArticulo As String, DescripcionArt As String) As ItemType
        ...
        'ItemClassificationCode.Value = IdArticulo.Trim
        ...
        Item.CommodityClassification = {CommodityClassification}
        Return Item
    End Function
    Private Function PrecioUnitario(ByVal PriceTypeCode_string As String, precioUnit As Decimal) As PriceType
        ...
        PriceTypeCode.listName = "SUNAT:Indicador de Tipo de Precio"
